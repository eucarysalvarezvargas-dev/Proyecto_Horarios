...
public class DocenteController extends ControladorBase {
        ...
        try {
            if (jdbc) {
                ...
                if (!vincularUsuarioDocente(creado)) {
                    try {
                        docenteDAO.eliminarLogico(creado.getIdDocente());
                    } catch (SQLException exRollback) {
                        setUltimoError("No se pudo crear la cuenta del docente y falló la reversión del registro.");
                        return null;
                    }
                    String errUsuario = usuarioController != null ? usuarioController.getUltimoError() : null;
                    setUltimoError(errUsuario != null ? errUsuario
                            : "No se pudo crear la cuenta de acceso del docente.");
                    auditarCrud(PermisosRol.MOD_DOCENTES, BitacoraController.ACCION_CREATE,
                            getUltimoError(), false);
                    return null;
                }
                auditarCrud(PermisosRol.MOD_DOCENTES, BitacoraController.ACCION_CREATE,
    ...
    private boolean vincularUsuarioDocente(Docente docente) {
        ...
        return usuarioController.registrarSinVinculo(u) != null;
    }
